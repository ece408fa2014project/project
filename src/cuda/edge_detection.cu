...
void do_edge_detection_cuda(float * r, float * g, float * b, float * out, float * prevFrame1, float * prevFrame2, float * prevFrame3, int width, int height) {
    ...
    if(prevFrame1 != NULL)
        diff_kernel<<<dim_grid_gray, dim_block_gray>>>(grad_dev, prevFrame1, average_dev_1, width, height);
    if(prevFrame2 != NULL)
        diff_kernel<<<dim_grid_gray, dim_block_gray>>>(average_dev_1, prevFrame2, average_dev_2, width, height);
    if(prevFrame3 != NULL)
